...
const InviteStep = ({ onComplete }: InviteStepProps) => {
  ...
  const bump = () => setShareCount((c) => Math.min(c + 1, INVITE_GOAL));
      ...
      <div className="flex items-center gap-2">
        {Array.from({ length: INVITE_GOAL }).map((_, i) => (
          <motion.div
            ...
            initial={{ scale: 0.5, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ ...signalTransition, delay: 0.2 + i * 0.08 }}
            className={`h-7 w-7 rounded-full flex items-center justify-center text-[11px] font-medium transition-all ${
              i < progress
        ...
        {progress >= INVITE_GOAL ? (
          ...
          >
            ...
            <motion.button
              whileTap={{ scale: 0.97 }}
              onClick={handleShare}
              className="w-full rounded-2xl bg-primary py-3.5 text-[13px] font-medium text-primary-foreground"
            >
            ...
            <div className="flex items-center justify-center gap-2">
              {channelButtons.map((btn) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.92 }}
                  onClick={btn.action}
                  className="flex items-center gap-1.5 rounded-xl border border-border/40 px-3 py-2 text-[11px] text-muted-foreground transition-colors hover:text-foreground hover:border-primary/30"
                >
                  {btn.icon}
                  {btn.label}
                </motion.button>
